...
const PUBLIC_FEE_DENOM = process.env.NEXT_PUBLIC_FEE_DENOM || "ucosm";
const PUBLIC_CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CONTRACT_ADDRESS || "wasm1fyvnk5zxh87n8f090wty327vl8y2eeadt0d60kmy3t486zzmzm6qwxvrnn";
...
const Multiple: NextPage = () => {
  ...
  const [parsedCsvData, setParsedCsvData] = useState<Edge[]>([]);
  ...
  const handleMultiple = (event: MouseEvent<HTMLElement>) => {
    ...
    // const messages = parsedCsvData.map((Edge[])) => 
    ...
    const messages: any =  
      parsedCsvData.map(({ creditor, amount }) => 
      ({
    ...
    signingClient
    ?.executeMultiple(walletAddress, messages,"auto" )    
    .then((resp) => {
